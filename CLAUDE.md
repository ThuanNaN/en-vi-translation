...
**Build status:** Phase 2 (model + Triton serving) and Phase 3 (queue + async API) are
complete. `src/envit5/worker/` and `src/envit5/api/` are implemented; all 18 API tests pass.
Cross-cutting work (Prometheus metrics, structured errors, retries/timeouts) is Phase 4.
...
- **Config is centralized** in `src/envit5/core/settings.py` (pydantic-settings, env prefix
  ...
  directly.
- **API contract (Phase 3 target):** `POST /translate` (returns `job_id`) + `GET /jobs/{id}`
  (poll for result). Auth via `X-API-Key` header. Direction via `source`/`target` fields or the
  `direction` shorthand (`"en-vi"`); source auto-detected when both omitted.
...
make all        # build + export + up + observe
```
...
```bash
pytest                        # run all tests (api tests auto-skip until Phase 3 is implemented)
...
- **Test isolation:** `tests/conftest.py` patches all service URLs to safe defaults and calls
  ...
  `@lru_cache`. The api tests use `pytest.importorskip("envit5.api.app")` so they silently skip
