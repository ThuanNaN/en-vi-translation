...
- **Config is centralized** in `src/envit5/core/settings.py` (pydantic-settings, env prefix
  `ENVIT5_`). Don't read env vars ad hoc elsewhere; add a field there.
...
torch locally). Override model ids via `ENVIT5_HF_MODEL_EN_VI` / `ENVIT5_HF_MODEL_VI_EN`
(see `.env.example`). Bump `TRITON_VERSION` if the default base image tag isn't on nvcr.io.
...
- Lint with `ruff` (config in `pyproject.toml`); install dev tools with `pip install -e '.[dev]'`.
